...
interface DiffViewerProps {
  tab: { path: string; content: string | unknown | null; name: string };
  onClose?: () => void;
}

const DiffViewer = ({ tab, onClose }: DiffViewerProps) => {
  const diffText = typeof tab.content === 'string' ? tab.content : '';
  const filePath = tab.path;
  const lines = useMemo(() => parseDiff(diffText), [diffText]);
  const fileName = filePath ? filePath.split('/').pop() : tab.name;
